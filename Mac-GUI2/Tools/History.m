...
- (void) commit: (NSString*) entry {
    ...
    if (ac==0 || ![[hist objectAtIndex: ac-1] isEqualToString:entry]) {
		len = [entry length]; 		
		if ([Preferences flagForKey:kStripCommentsFromHistoryEntries withDefault:NO]) {
			int i = 0, j = 0, k = 0;
		...
		len = [entry length];
		if ([Preferences flagForKey:kCleanupHistoryEntries withDefault:YES]) {
			while (len > 1 && 
		...
		if (!(len == 1 &&
			  ...
			  [entry characterAtIndex:len-1] == '#')) {
			if (len > 0 ) {
				if ([Preferences flagForKey:kRemoveDuplicateHistoryEntries withDefault:NO])
    ...
    dirtyEntry = nil;
	pos = [hist count];
	int max = [[Preferences stringForKey:kMaxHistoryEntries withDefault:@"250"] intValue];
	if (max > 0 && [hist count] > max) 
