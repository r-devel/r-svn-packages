			    ...
			    gpointer data) 
{
  ...
  gtk_layout_freeze(GTK_LAYOUT(gd->canvas));
  return FALSE;
}

static gint check_resize_cb(GtkWidget *widget, gpointer data)
{
	printf("Check resize!\n");
	return FALSE;
}

static gint size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation,
			     gpointer data) 
{
  NewDevDesc *dd;
  gnomeDesc *gd;
  ...
  if((gd->windowWidth != allocation->width) || 
     (gd->windowHeight != allocation->height))
  {
	  printf("Resizing\n");
	  gd->windowWidth = allocation->width;
	  gd->windowHeight = allocation->height;
	  gd->resize = TRUE;
  }
...
static gint ebox_expose_event(GtkWidget *widget, GdkEventExpose *event,
				gpointer data)
{
	printf("EventBox exposed\n");
	return FALSE;
}
			 ...
			 gpointer data)
{
  ...
  printf("Expose event\n");
  gtk_layout_thaw(GTK_LAYOUT(gd->canvas));
...
GNOME_Open(NewDevDesc *dd, gnomeDesc *gd, char *dsp, double w, double h)
{
    ...
    /* create toolbar 
       - commented out when gnome device unbundled
    ...
    */
    ...
    gtk_widget_set_events(gd->canvas,
			  GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK);
    
    ebox = gtk_event_box_new();
    gtk_widget_set_events(GTK_WIDGET(ebox), GDK_EXPOSURE_MASK
	    | GDK_BUTTON_PRESS_MASK);
    gtk_container_add(GTK_CONTAINER(gd->window), ebox);
    gtk_widget_realize(ebox);

    /* place and realize the canvas */
    //gtk_container_add(GTK_CONTAINER(gd->window), gd->canvas);
    gtk_container_add(GTK_CONTAINER(ebox), gd->canvas);
    ...
    /*
    gtk_signal_connect(GTK_OBJECT(gd->window), "size-allocate",
		       (GtkSignalFunc) size_allocate_cb, (gpointer) dd);
    */
    ...
    gtk_signal_connect(GTK_OBJECT(gd->canvas), "check-resize",
		       (GtkSignalFunc) check_resize_cb, (gpointer) dd);
		     ...
		     NewDevDesc *dd)
{
    ...
    gnome_canvas_update_now(GNOME_CANVAS(gd->canvas));
//    gdk_flush();
}
