...
/* Forms product X_i'WX_j where X_i and X_j are stored in compact form. 
   ...
   * off[i] is offset to start of ith matrix (out of nx)  
   ...
   This version works. It has the loop over X, C and D columns outer to the loop over data. This looks inefficient, 
   but is much more Cache friendly than the alternative ordering as the required matrix columns stay in Cache. 

*/
  int si,sj,ri,rj,jm,im,kk,ddt,ddtj,koff,*K,*Ki,*Kj,pim,pjm,
    ii,jj,rfac,q,t,s,ddti,tensi,tensj,acc_w,alpha,*Kik,*Kjk,*Kik1,*Kjk1;
  ptrdiff_t mim,mjm; /* avoid integer overflow in large pointer calculations */ 
  double x,*wl,*dXi,*dXj,*pdXj,*Xt,*Xi,*Xj,*Cq,*Dq,done=1.0,dzero=0.0,
    *C,*D,*W,*wb,*p0,*p1,*p2,*p3,*pw,*pw1,*pl,*ps;
  char trans = 'T',ntrans = 'N';
  si = ks[ts[i]+nx]-ks[ts[i]]; /* number of terms in summation convention for i */
  if (tri) wl = ws + n - 1; /* sub-diagonal */
  /* compute number of columns in dXi/ number of rows of blocks in product */
...
void XWXij_q(double *XWX,int i,int j,int r,int c, double *X,int *k, int *ks, int *m, int *p,int nx,int n,int *ts, int *dt,
	   int nt, double *w,double *ws, int tri,ptrdiff_t *off,double *work,int nxwx) {
/* Forms product X_i'WX_j where X_i and X_j are stored in compact form. 
   * W = diag(w) if tri!=0 and is tridiagonal otherwise, with super in ws and sub in ws + n-1;     
   * off[i] is offset to start of ith matrix (out of nx)  
   ...
   This version works across the columns of C and D within the loop over data loop, but it is very cache 
   inefficient because of the random access to X, C and D. 

*/
  int si,sj,ri,rj,jm,im,kk,ddt,ddtj,koff,*K,*Ki,*Kj,pim,pjm,
    ii,jj,rfac,t,s,ddti,tensi,tensj,acc_w,alpha,*Kik,*Kjk,*Kik1,*Kjk1;
  ptrdiff_t mim,mjm; /* avoid integer overflow in large pointer calculations */ 
  double x,x1,x2,*wl,*dXi,*dXj,*pdXj,*Xt,*Xi,*Xj,*Cq,*Dq,done=1.0,dzero=0.0,
    *C,*D,*W,*wb,*p0,*p1,*p2,*p3,*pw,*pw1,*pl,*ps,*Cend,*Xj0,*Cq1,*Xj1,*Dq1,*Dend,*Xi0,*Xi1;
  char trans = 'T',ntrans = 'N';
  si = ks[ts[i]+nx]-ks[ts[i]]; /* number of terms in summation convention for i */
  if (tri) wl = ws + n - 1; /* sub-diagonal */
  /* compute number of columns in dXi/ number of rows of blocks in product */
	  ...
	  if (acc_w) { /* weight accumulation */
		...
		W[*Kik + mim * *Kjk] += *pw;
	      ...
	      //for (kk=0;kk<n;kk++) W[Ki[kk]+mim*Kj[kk]] += w[kk];
            ...
            //for (q=0;q<p[jm];q++) { /* !!THIS IS INSANE!! The q loop should be inside the k loop --- currently a bunch of the same work is pointlessly repeated for each column.  */
	    //  Cq = C + q * mim;Xj = X + off[jm] + q * mjm; /* qth cols of C and X */
	      Cend = C + mim * p[jm];Xj0 = X + off[jm];
	      if (tensi&&tensj) { /* X_i and X_j are tensors */
	        // for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*dXi[kk]*dXj[kk]*Xj[Kj[kk]];
		// for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p2++,p3++) Cq[*Kik] += *p0 * *p2 * *p3 * Xj[*Kjk];
	        //if (tri) for (kk=0;kk<n-1;kk++) {
		...
		if (tri) {
		  for (Kik=Ki,Kjk=Kj,Kjk1=Kj+1,Kik1=Ki+1,ps=ws,pw=w,pw1=w+n-1,pl=wl,p0=dXi,p1=dXj,p2=dXi+1,p3=dXj+1;pw<pw1;
		       Kik++,Kik1++,Kjk++,Kjk1++,ps++,pw++,pl++,p0++,p1++,p2++,p3++) {
		    Cq = C + *Kik;Cq1 = C + *Kik1;Xj=Xj0 + *Kjk;Xj1 = Xj0 + *Kjk1;
		    x = *p0 * *ps * *p3;x1 = *p0 * *pw * *p1;x2 = *pl * *p2 * *p1;
		    for (;Cq<Cend;Cq+=mim,Cq1+=mim,Xj+=mjm,Xj1+=mjm) { *Cq += x * *Xj1 + x1 * *Xj;*Cq1 += x2 * *Xj;}
		  }
		  Cq = C + *Kik;Xj=Xj0 + *Kjk;x1 = *p0 * *pw * *p1;
		  for (;Cq<Cend;Cq+=mim,Xj+=mjm) *Cq += x1 * *Xj;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p2++,p3++)
			 for (x= *p0 * *p2 * *p3,Cq=C + *Kik,Xj = Xj0 + *Kjk;Cq<Cend;Cq += mim,Xj += mjm) *Cq += x * *Xj;
		// for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*dXi[kk]*dXj[kk]*Xj[Kj[kk]];
	      } else if (tensi) { /* only X_i is tensor */
		//for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*dXi[kk]*Xj[Kj[kk]];
		// for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi;p0<p1;p0++,Kik++,Kjk++,p2++) Cq[*Kik] += *p0 * *p2  * Xj[*Kjk];
		...
		if (tri) {
		  for (Kik=Ki,Kjk=Kj,Kjk1=Kj+1,Kik1=Ki+1,ps=ws,pw=w,pw1=w+n-1,pl=wl,p0=dXi,p2=dXi+1;pw<pw1;
		       Kik++,Kik1++,Kjk++,Kjk1++,ps++,pw++,pl++,p0++,p2++) {
		    Cq = C + *Kik;Cq1 = C + *Kik1;Xj=Xj0 + *Kjk;Xj1 = Xj0 + *Kjk1;
		    x = *p0 * *ps;x1 = *p0 * *pw;x2 = *pl * *p2;
		    for (;Cq<Cend;Cq+=mim,Cq1+=mim,Xj+=mjm,Xj1+=mjm) { *Cq += x * *Xj1 + x1 * *Xj;*Cq1 += x2 * *Xj;}
		  }
		  Cq = C + *Kik;Xj=Xj0 + *Kjk;x1 = *p0 * *pw;
		  for (;Cq<Cend;Cq+=mim,Xj+=mjm) *Cq += x1 * *Xj;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi;p0<p1;p0++,Kik++,Kjk++,p2++)
                       for (x= *p0 * *p2,Cq=C + *Kik,Xj = Xj0 + *Kjk;Cq<Cend;Cq += mim,Xj += mjm) *Cq += x * *Xj;
	        //for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*dXi[kk]*Xj[Kj[kk]];
	      } else if (tensj) { /* only X_j is tensor */
	        //for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*dXj[kk]*Xj[Kj[kk]];
		//for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p3++) Cq[*Kik] += *p0 * *p3 * Xj[*Kjk];
		...
		if (tri) {
		  for (Kik=Ki,Kjk=Kj,Kjk1=Kj+1,Kik1=Ki+1,ps=ws,pw=w,pw1=w+n-1,pl=wl,p1=dXj,p3=dXj+1;pw<pw1;
		       Kik++,Kik1++,Kjk++,Kjk1++,ps++,pw++,pl++,p1++,p3++) {
		    Cq = C + *Kik;Cq1 = C + *Kik1;Xj=Xj0 + *Kjk;Xj1 = Xj0 + *Kjk1;
		    x =  *ps * *p3;x1 =  *pw * *p1;x2 = *pl  * *p1;
		    for (;Cq<Cend;Cq+=mim,Cq1+=mim,Xj+=mjm,Xj1+=mjm) { *Cq += x * *Xj1 + x1 * *Xj;*Cq1 += x2 * *Xj;}
		  }
		  Cq = C + *Kik;Xj=Xj0 + *Kjk;x1 =  *pw * *p1;
		  for (;Cq<Cend;Cq+=mim,Xj+=mjm) *Cq += x1 * *Xj;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p3++)
			 for (x= *p0  * *p3,Cq=C + *Kik,Xj = Xj0 + *Kjk;Cq<Cend;Cq += mim,Xj += mjm) *Cq += x * *Xj;
		//for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*dXj[kk]*Xj[Kj[kk]];
	      } else { /* both singletons */
		//for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*Xj[Kj[kk]];
		//for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n;p0<p1;p0++,Kik++,Kjk++) Cq[*Kik] += *p0 * Xj[*Kjk];
		...
		if (tri) {
		  ...
		  Cq = C + *Kik;Xj=Xj0 + *Kjk;x1 =  *pw;
		  for (;Cq<Cend;Cq+=mim,Xj+=mjm) *Cq += x1 * *Xj;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n;p0<p1;p0++,Kik++,Kjk++)
                   for (x= *p0,Cq=C + *Kik,Xj = Xj0 + *Kjk;Cq<Cend;Cq += mim,Xj += mjm) *Cq += x * *Xj;
		//for (kk=0;kk<n;kk++) Cq[Ki[kk]] += w[kk]*Xj[Kj[kk]];
	      }
	      //} /* q loop */
	  } else { /* direct accumulation of left factor D (mjm by p[im]) = \bar W' X_i */
	    //Rprintf("D ");
            //for (q=0;q<p[im];q++) {
	      Dend = D + p[im] * mjm;Xi0 = X + off[im]; /* qth cols of C and Xi */ 
	      if (tensi&&tensj) { /* X_i and X_j are tensors */
	        // for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*dXi[kk]*dXj[kk]*Xi[Ki[kk]];
		//for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p2++,p3++) Dq[*Kjk] += *p0 * *p2 * *p3 * Xi[*Kik];
	        //if (tri) for (kk=0;kk<n-1;kk++) {
		...
		if (tri) {
		  for (Kik=Ki,Kjk=Kj,Kjk1=Kj+1,Kik1=Ki+1,ps=ws,pw=w,pw1=w+n-1,pl=wl,p0=dXi,p1=dXj,p2=dXi+1,p3=dXj+1;pw<pw1;
		       Kik++,Kik1++,Kjk++,Kjk1++,ps++,pw++,pl++,p0++,p1++,p2++,p3++) {
		    Dq = D + *Kjk;Dq1 = D + *Kjk1;Xi=Xi0 + *Kik;Xi1 = Xi0 + *Kik1;
		    x = *p1 * *p2 * *pl;x1 = *p0 * *pw * *p1;x2 = *ps * *p0 * *p3;
		    for (;Dq<Dend;Dq+=mjm,Dq1+=mjm,Xi+=mim,Xi1+=mim) { *Dq += x * *Xi1 + x1 * *Xi;*Dq1 += x2 * *Xi;}
		  }
		  Dq = D + *Kjk;Xi=Xi0 + *Kik;x1 = *p0 * *pw * *p1;
		  for (;Dq<Dend;Dq+=mjm,Xi+=mim) *Dq += x1 * *Xi;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p2++,p3++)
			 for (x= *p0 * *p2 * *p3,Dq=D + *Kjk,Xi = Xi0 + *Kik;Dq<Dend;Dq += mjm,Xi += mim) *Dq += x * *Xi;
		// for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*dXi[kk]*dXj[kk]*Xi[Ki[kk]];
	      } else if (tensi) { /* only X_i is tensor */
	        //for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*dXi[kk]*Xi[Ki[kk]];
		//for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi;p0<p1;p0++,Kik++,Kjk++,p2++) Dq[*Kjk] += *p0 * *p2  * Xi[*Kik];
		...
		if (tri) {
		  for (Kik=Ki,Kjk=Kj,Kjk1=Kj+1,Kik1=Ki+1,ps=ws,pw=w,pw1=w+n-1,pl=wl,p0=dXi,p2=dXi+1;pw<pw1;
		       Kik++,Kik1++,Kjk++,Kjk1++,ps++,pw++,pl++,p0++,p2++) {
		    Dq = D + *Kjk;Dq1 = D + *Kjk1;Xi=Xi0 + *Kik;Xi1 = Xi0 + *Kik1;
		    x =  *p2 * *pl;x1 = *p0 * *pw ;x2 = *ps * *p0 ;
		    for (;Dq<Dend;Dq+=mjm,Dq1+=mjm,Xi+=mim,Xi1+=mim) { *Dq += x * *Xi1 + x1 * *Xi;*Dq1 += x2 * *Xi;}
		  }
		  Dq = D + *Kjk;Xi=Xi0 + *Kik;x1 = *p0 * *pw ;
		  for (;Dq<Dend;Dq+=mjm,Xi+=mim) *Dq += x1 * *Xi;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p2=dXi;p0<p1;p0++,Kik++,Kjk++,p2++)
                        for (x= *p0 * *p2,Dq=D + *Kjk,Xi = Xi0 + *Kik;Dq<Dend;Dq += mjm,Xi += mim) *Dq += x * *Xi;
		//for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*dXi[kk]*Xi[Ki[kk]];
	      } else if (tensj) { /* only X_j is tensor */
	        //for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*dXj[kk]*Xi[Ki[kk]];
		//for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p3++) Dq[*Kjk] += *p0 * *p3 * Xi[*Kik];
		...
		if (tri) {
		  for (Kik=Ki,Kjk=Kj,Kjk1=Kj+1,Kik1=Ki+1,ps=ws,pw=w,pw1=w+n-1,pl=wl,p1=dXj,p3=dXj+1;pw<pw1;
		       Kik++,Kik1++,Kjk++,Kjk1++,ps++,pw++,pl++,p1++,p3++) {
		    Dq = D + *Kjk;Dq1 = D + *Kjk1;Xi=Xi0 + *Kik;Xi1 = Xi0 + *Kik1;
		    x = *p1  * *pl;x1 = *pw * *p1;x2 = *ps * *p3;
		    for (;Dq<Dend;Dq+=mjm,Dq1+=mjm,Xi+=mim,Xi1+=mim) { *Dq += x * *Xi1 + x1 * *Xi;*Dq1 += x2 * *Xi;}
		  }
		  Dq = D + *Kjk;Xi=Xi0 + *Kik;x1 =  *pw * *p1;
		  for (;Dq<Dend;Dq+=mjm,Xi+=mim) *Dq += x1 * *Xi;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n,p3=dXj;p0<p1;p0++,Kik++,Kjk++,p3++)
                   for (x= *p0 * *p3,Dq=D + *Kjk,Xi = Xi0 + *Kik;Dq<Dend;Dq += mjm,Xi += mim) *Dq += x * *Xi;
		//for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*dXj[kk]*Xi[Ki[kk]];
	      } else { /* both singletons */
	        //for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*Xi[Ki[kk]];
		//for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n;p0<p1;p0++,Kik++,Kjk++) Dq[*Kjk] += *p0 * Xi[*Kik];
		...
		if (tri) {
		  ...
		  Dq = D + *Kjk;Xi=Xi0 + *Kik;x1 =  *pw;
		  for (;Dq<Dend;Dq+=mjm,Xi+=mim) *Dq += x1 * *Xi;
		} else for (Kik=Ki,Kjk=Kj,p0=w,p1=w+n;p0<p1;p0++,Kik++,Kjk++)
                    for (x= *p0,Dq=D + *Kjk,Xi = Xi0 + *Kik;Dq<Dend;Dq += mjm,Xi += mim) *Dq += x * *Xi;
		//for (kk=0;kk<n;kk++) Dq[Kj[kk]] += w[kk]*Xi[Ki[kk]];
	      }
	      // } /* q loop */
	  }  
...
void XWXd0_best(double *XWX,double *X,double *w,int *k,int *ks, int *m,int *p, int *n, int *nx, int *ts, 
          int *dt, int *nt,double *v,int *qc,int *nthreads,int *ar_stop,int *ar_row,double *ar_weights) {
/* essentially a driver routine for XWXij implementing block oriented cross products
...
*/   
  int *pt, *pd,i,q,j,si,maxm=0,maxp=0,maxmp=0,tri,r,c,rb,cb,rt,ct,pa,*tps,*tpsu,ptot,*b,*B,*C,*R,*sb,N,kk,kb,tid=0,nxwx=0;
  ptrdiff_t *off,*voff,mmp;
