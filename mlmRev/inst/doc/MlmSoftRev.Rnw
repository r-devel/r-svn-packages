...
mcmc2frm <- function(obj)
{
    nc <- ncol(obj)
    nr <- nrow(obj)
    data.frame(vals = c(obj), iter = rep(1:nr, nc),
               par = factor(rep(1:nc, each = nr), labels =
               colnames(obj)))
}
print(densityplot(~vals | par, mcmc2frm(Es4),
                  plot = FALSE, ylab = "", xlab = "",
                  layout = c(4,2),
...
print(qqmath(~vals | par, mcmc2frm(Es4),
             ylab = "",
...
print(densityplot(~vals | par, mcmc2frm(Es4a),
                  plot = FALSE, ylab = "", xlab = "",
                  layout = c(4,2),
                  scales = list(relation = "free",
                  x = list(axs = "i"))))
...
print(qqmath(~vals | par, mcmc2frm(Es4a),
             ylab = "",
             xlab = "Quantiles of a standard normal distribution",
             layout = c(2,2),
             scales = list(y = list(relation = "free"),
...
<<Cm7>>=
(Cm7 <- lmer(use ~ age + I(age^2) + urban + (livch == 0) + (1|district),
             Contraception, binomial, method = "AGQ"))
anova(Cm6, Cm7)
@ 
...
<<children>>=
Contraception$children <- with(Contraception, factor(ifelse(livch == 0, "N", "Y")))
@ 
...
<<Cm11>>=
(Cm11 <- lmer(use ~ age + I(age^2) + urban + children + (1|district), Contraception, binomial, method = "AGQ"))
Cs11 <- mcmcsamp(Cm11, 10000)
@ 
Density plots (Figure~\ref{fig:Contra2})
<<Contra2,fig=TRUE,echo=FALSE,width=8,height=5,include=FALSE>>=
print(densityplot(~vals | par, mcmc2frm(Cs11),
                  plot = FALSE, ylab = "", xlab = "",
                  layout = c(3,2),
                  scales = list(relation = "free",
                  x = list(axs = "i"))))
...
print(qqmath(~vals | par, mcmc2frm(Cs11),
             ylab = "",
             xlab = "Quantiles of a standard normal distribution",
             layout = c(3,2),
             scales = list(y = list(relation = "free"),
