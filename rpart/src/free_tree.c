...
void
free_tree(struct node *node, int freenode)
{
    if (node->rightson)
    ...
    if (freenode == 1)
       ...
       /* don't point to things I just freed */
        node->primary = (struct split *) NULL;
        node->surrogate = (struct split *) NULL;
        node->rightson = (struct node *) NULL;
        node->leftson = (struct node *) NULL;
    }
}
