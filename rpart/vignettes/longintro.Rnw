...
options(SweaveHooks=list(fig=function() par(mar=c(4.1, 4.1, .1, 1.1))))
...
plot(fit1, margin=.05);  text(fit1,use.n=T,cex=.8)
plot(fit2, margin=.05);  text(fit2,use.n=T,cex=.8)
@ 
...
lmat <- matrix(c(0,3,4,0), nrow=2, ncol=2, byrow=F)
...
plot(fit1);  text(fit1,use.n=T,all=T, cex=.8)
plot(fit2);  text(fit2,use.n=T,all=T, cex=.8)
plot(fit3);  text(fit3,use.n=T,all=T, cex=.8)
@ 
...
<<>>=
sfit <- rpart(skips ~ Opening + Solder + Mask + PadType 
...
par(mar=rep(.1,4))
plot(sfit)
text(sfit,use.n=T, min=3)

fit.prune <- prune(sfit,cp=.10)
plot(fit.prune)
text(fit.prune,use.n=T,min=2)
@ 

The \Co{use.n=T} option specifies that number of events / total N should be
...
\Co{cp = .10} in the original call to {\rpart}.
...
pfit2 <- prune(pfit, cp=.016)
par(mar=rep(.2,4))
plot(pfit2, uniform=T, branch=.4, compress=T)
text(pfit2, use.n=T)
@ 
...
\Co{prune(fit, cp=.016)} would give the desired result,
...
legend(10, .3, paste('node',c(4,5,6,7)), lty=1:4)
@ 
...
\begin{figure}
  ...
  \caption{\Co {plot(fit, uniform=T); text(fit,use.n=T,all=T)}}
  \label{plotfig2}
\end{figure}

The next plot has uniform stem lengths (\Co{uniform=T}), has extra 
information (\Co{use.n=T}) specifying number of subjects at each node, 
and has labels on all the nodes, not just the terminal nodes (\Co{all=T}).
...
\begin{figure}
  ...
  \caption{\Co{plot(fit, branch=0); text(fit,use.n=T)}}
  \label{plotfig3}
\end{figure}
...
\begin{figure}
  ...
  \caption{\Co{plot(fit, branch=.4, uniform=T,compress=T)}}
  \label{plotfig4}
\end{figure}
...
par(mar=rep(.2,4))
plot(fit, uniform=T, branch = 0.2, compress=TRUE, margin=.1)
text(fit, all=T, use.n=T, fancy=T, cex=.9)
@ 
